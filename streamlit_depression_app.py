    ...
    with col2:
        if 'last_prediction' in st.session_state:
            ...
            prob_df = pd.DataFrame({
                'Class': [inverted_target_map[i] for i in range(len(result['probabilities']))],
            ...
            if export_button:
                json_output = export_prediction(input_data, result)
                st.download_button(
                    label="Download Prediction JSON",
                    data=json_output,
                    file_name="depression_prediction.json",
